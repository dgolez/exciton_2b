...
mpi_lattice_step_RPA<LATTICE>::mpi_lattice_step_RPA(int nt,int ntau,int size,double beta,double h,bool use_omp_for_vie2,double mu,double epsilon,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &g,CFUNC &omega0,CFUNC &gBATH,double xi,std::vector<double> &delta,std::vector<double> &A,std::vector<double> &dA,double v01,int nk,int kt,bool test,double mix):
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::init_G_mat_nointeraction(void){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::extrapolate_timestep_G(int tstp,int kt){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::extrapolate_timestep_W(int tstp,int kt){
  assert(tstp<=this->nt_-1);
	// extrapolate from tstp to tstp+1
	if(tstp<-1) return;
	if(tstp==-1){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::symmetrise_mat(GREEN &G){
	cdmatrix M,M1;
...
template <class LATTICE>
double mpi_lattice_step_RPA<LATTICE>::step(int tstp,int iter,int kt,double om0,double s,double amp){
  assert(tstp<=this->nt_-1);
	double err;
	int n;
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::gather_gk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::set_density_k(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::gather_Wk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::gather_Dk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::get_Polarization_Bubble(int tstp,int qq,GREEN &P){
   assert(tstp==gk_all_timesteps_.tstp());	
	...
	for(int kk=0;kk<this->latt_.nk_bz_;kk++){
		...
		kq=this->latt_.add_kpoints(kk,1,qq,-1);
		...
		for(i1=0;i1<this->nrpa_;i1++){
			for(i2=0;i2<this->nrpa_;i2++){
			  cntr::Bubble1(tstp,ptmp,i1,i2,gk_all_timesteps_.G()[kq1],gk_all_timesteps_.G()[kq1],i1,i2,gk_all_timesteps_.G()[kk1],gk_all_timesteps_.G()[kk1],i1,i2);										  
			}
		}
		// increment P += -wt*Ptmp
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::get_Sigma_RPA(int tstp,int kk,GREEN &S,int iter){
  assert(tstp==gk_all_timesteps_.tstp());
  assert(tstp==wk_all_timesteps_.tstp());
	...
	for(int q=0;q<this->latt_.nk_bz_;q++){
		...
		for(i1=0;i1<this->nrpa_;i1++){
			for(i2=0;i2<this->nrpa_;i2++){
			  cntr::Bubble2(tstp,stmp,i1,i2,gk_all_timesteps_.G()[kq1],gk_all_timesteps_.G()[kq1],i1,i2,wk_all_timesteps_.G()[q1],wk_all_timesteps_.G()[q1],i1,i2);
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::get_Gloc(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA<LATTICE>::get_Wloc(int tstp){
	// collect the local Greenfunction at rank with id tid
	GREEN_TSTP gtmp(tstp,this->ntau_,this->nrpa_,1);
	for(int kk=0;kk<this->latt_.nk_bz_;kk++){
		double wt=this->latt_.kweight_bz_[kk];
...
template <class LATTICE>
double mpi_lattice_step_RPA<LATTICE>::get_eneRPA(int tstp,int kt){
	cdmatrix tmp,test;
