...
mpi_lattice_step_RPA_cdw<LATTICE>::mpi_lattice_step_RPA_cdw(int nt,int ntau,int size,double beta,double h,bool use_omp_for_vie2,double mu,double epsilon,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &g,CFUNC &omega0,double xi,std::vector<double> &delta,std::vector<double> &A,std::vector<double> &dA,double v01,int nk,int kt,bool test,double mix):
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::init_G_mat_nointeraction(void){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::extrapolate_timestep_G(int tstp,int kt){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::extrapolate_timestep_W(int tstp,int kt){
  assert(tstp<=this->nt_-1);
	// extrapolate from tstp to tstp+1
	if(tstp<-1) return;
	if(tstp==-1){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::symmetrise_mat(GREEN &G){
	cdmatrix M,M1;
...
template <class LATTICE>
double mpi_lattice_step_RPA_cdw<LATTICE>::step(int tstp,int iter,int kt,double om0,double s,double amp){
  assert(tstp<=this->nt_-1);
	double err;
	int n;
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::gather_gk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::set_density_k(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::gather_Wk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::gather_Dk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Sigma_Hartree_cdw(int tstp,int kk,CFUNC &S){
	...
	for(int k=0;k<latt_.nk_rbz_;k++){
		double wk=0.5*latt_.kweight_bz_[k]; // factor 2: ksum normalized for RBZ
		int i1,i2,c12,at,ct;
		cdmatrix rtmp;
		this->density_k_[latt_.representative_kk(k)].rho_.get_value(tstp,rtmp);
		//rtmp1+=wk*rtmp;
		for(i1=0;i1<nrpa_;i1++){
			for(i2=0;i2<nrpa_;i2++){
				...
				cdouble stmpij=0.0;
				c12=(c1+c2)%2; // really c1-c2, but it is the same
				if(a1==a2){
					for(at=0;at<norb_;at++){
						for(ct=0;ct<2;ct++){
							stmpij+=v0(latt_.idx(a1,c12),latt_.idx(at,c12))*
								        rtmp(latt_.idx(at,(c2+ct)%2),latt_.idx(at,(c1+ct)%2));
						}
					}
				}
				stmp(i1,i2) += wk*stmpij;
			}
		}
		S.set_value(tstp,stmp);
	}
	// Electron-phonon interaction
	// No multiplication with the density, since it's implicitly included
	// in the equation of motion for phonons
	// stmp+=Vtmp;
	// TODO: This is stupid implementation, 
	// since phonon class is defined and deleted twice per timestep. Do it just once per all program
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Sigma_Fock_cdw(int tstp,int kk,CFUNC &S){
	...
	for(int q=0;q<latt_.nk_rbz_;q++){
		...
		cdmatrix rtmp,vtmp;
		int kq,gammakq;
		//kq=latt_.add_kpoints(1,kk,-1,q);
		latt_.add_kpoints_rbz(kq,gammakq,1,kk,-1,q);
		gk_all_timesteps_.G()[latt_.representative_kk(kq)].density_matrix(tstp,rtmp);
		//if((kq-latt_.G_)!=0) rtmp.setZero();
		vertex_[latt_.representative_kk(q)].get_value(tstp,vtmp);
		//std::cout << "k="<< kk << " q= " << q <<"k-q=" << kq << " V00(q) " << vtmp(latt_.idx(0,0),latt_.idx(0,0)) << " V00(q+Q) " << vtmp(latt_.idx(0,1),latt_.idx(0,1))<< std::endl;
		for(i1=0;i1<nrpa_;i1++){
			for(i2=0;i2<nrpa_;i2++){
				int a1=latt_.idx_a(i1);
				int c1=latt_.idx_c(i1);
				...
				cdouble stmpij=0.0;
				for(ct=0;ct<2;ct++){
					stmpij -= vtmp(latt_.idx(a1,ct),latt_.idx(a2,ct))*
					          rtmp(latt_.idx(a1,(c1+ct+gammakq)%2),latt_.idx(a2,(c2+ct+gammakq)%2));						
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Sigma_RPA_cdw(int tstp,int kk,GREEN &S,int iter){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Polarization_Bubble(int tstp,int qq,GREEN &P){
   assert(tstp==gk_all_timesteps_.tstp());	
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Gloc(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_RPA_cdw<LATTICE>::get_Wloc(int tstp){
	// collect the local Greenfunction at rank with id tid
	GREEN_TSTP gtmp(tstp,this->ntau_,this->nrpa_,1);
	for(int kk=0;kk<this->latt_.nk_rbz_;kk++){
		double wt=this->latt_.kweight_bz_[kk];
...
template <class LATTICE>
double mpi_lattice_step_RPA_cdw<LATTICE>::get_eneRPA(int tstp,int kt){
	cdmatrix tmp,test;
