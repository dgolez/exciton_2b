...
mpi_lattice_step_hubbard_RPA<LATTICE>::mpi_lattice_step_hubbard_RPA(parameters_hub &param):
	base_type::mpi_lattice_step_hubbard(param){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::init_G_mat_nointeraction(void){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::extrapolate_timestep_G(int tstp,int kt){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::extrapolate_timestep_W(int tstp,int kt){
  // assert(tstp<=this->nt_-1);
	// extrapolate from tstp to tstp+1
	if(tstp<-1) return;
	if(tstp==-1){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::extrapolate_timestep_D(int tstp,int kt){
  // assert(tstp<=this->nt_-1);
	// extrapolate from tstp to tstp+1
	if(tstp<-1) return;
	if(tstp==-1){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::symmetrise_mat(GREEN &G){
	cdmatrix M,M1;
...
template <class LATTICE>
double mpi_lattice_step_hubbard_RPA<LATTICE>::step(int tstp,int iter,int kt,double om0,double s,double amp){
  // assert(tstp<=this->nt_-1);
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::gather_gk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::set_density_k(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::gather_Wk_timestep(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::gather_Dk_timestep(int tstp){
...
//get P_q (a1,a2) = -ii * sum_k *G_k(a1,a2;t,t')G_{q-k}(a2,a1;t,t')
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_Bubble(int tstp,int qq,GREEN &P){
  assert(tstp==gk_all_timesteps_.tstp());	
	...
	for(int kk=0;kk<this->latt_.nk_;kk++){
		double wk=this->latt_.kweight_bz_[kk];
		int gammakq;
		// get bubble for 2b approximation
		qk=this->latt_.add_kpoints(kk,-1,qq,1);
		ptmp.clear();

		// BUBBLE2 IS C_{c1,c2}(t1,t2) = ii * A_{a1,a2}(t1,t2) * B_{b1,b2}(t1,t2) 
		for(int i1=0;i1<this->nrpa_;i1++){
			for(int i2=0;i2<this->nrpa_;i2++){
				cntr::Bubble2(tstp,ptmp,i1,i2,gk_all_timesteps_.G()[kk],gk_all_timesteps_.G()[kk],i1,i2,gk_all_timesteps_.G()[qk],gk_all_timesteps_.G()[qk],i2,i1);
			}
		}	
		// The sign is different for the pp channel and ph channel [here ph channel]
		// Factor two comes from the spin sum
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_phonon_Bubble(int tstp,int qq,GREEN &P){
  assert(tstp==gk_all_timesteps_.tstp());	
	// compute P_{a,a'}(q;t,t') on a timestep
	int qk,i1,i2,qk1,kk1;
...
// \Sig_{a1,a2}(k,t,t')=i\sum_q G_{a1,a2}(k-q,t,t') W_{a2,a1}(q,t,t')

template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_Sigma_rpa(int tstp,int kk,GREEN &S,int iter){
  assert(tstp==gk_all_timesteps_.tstp());
  assert(tstp==wk_all_timesteps_.tstp());
	...
	for(int q=0;q<this->latt_.nk_;q++){
		double wk=this->latt_.kweight_bz_[q];
		int kq=this->latt_.add_kpoints(kk,1,q,-1);
		stmp.clear();

		for(int i1=0;i1<this->nrpa_;i1++){
			for(int i2=0;i2<this->nrpa_;i2++){
				cntr::Bubble2(tstp,stmp,i1,i2,gk_all_timesteps_.G()[kq],gk_all_timesteps_.G()[kq],i1,i2,wk_all_timesteps_.G()[q],wk_all_timesteps_.G()[q],i2,i1);
			}
		}
		...
		//Add bath
		S.incr_timestep(tstp,Bath_,wk);
	}
}

template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_Gloc(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_Wloc(int tstp){
	// collect the local Greenfunction at rank with id tid
	GREEN_TSTP gtmp(tstp,this->ntau_,this->nrpa_,1);
	for(int kk=0;kk<this->latt_.nk_;kk++){
		double wt=this->latt_.kweight_bz_[kk];
...
template <class LATTICE>
void mpi_lattice_step_hubbard_RPA<LATTICE>::get_Dloc(int tstp){
	// collect the local Greenfunction at rank with id tid
...
template <class LATTICE>
double mpi_lattice_step_hubbard_RPA<LATTICE>::get_eneRPA(int tstp,int kt){
	cdmatrix tmp(this->nrpa_,this->nrpa_);
	...
	// add all to task tid
		...
		// MPI_COMM_WORLD.Allreduce(&eRPAtmp,&eRPA,1,MPI_DOUBLE,MPI_SUM);
