...
int main(int argc,char *argv[]){
	...
	approx<lattice_1d_2b_nofield> *lattice;
	int approximation;
	...
	switch(approximation){
		case 0 : lattice=new mpi_lattice_step<lattice_1d_2b_nofield>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,gBATH,xi,delta,Ak,dAk,v01,nk,kt);break;
		...
		case 3 : lattice=new mpi_lattice_step_2b<lattice_1d_2b_nofield>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,gBATH,xi,delta,Ak,dAk,v01,nk,kt,test,mix);break;
	}
	// TODO
	if(restart_time!=-1){
	...
	std::ofstream out;
	out.open("disper.out");
	for(int k=0;k<lattice->latt_.nk_bz_;k++){
		Eigen::MatrixXcd vtmp;
		out << lattice->latt_.kpoints_bz_[k];
		lattice->latt_.hk(vtmp,-1,lattice->latt_.kpoints_bz_[k],1);
		out << " " << vtmp(0,0).real() << " " << vtmp(1,1).real()  <<  " ";
		lattice->latt_.hkfree(vtmp,-1,lattice->latt_.kpoints_bz_[k]);
