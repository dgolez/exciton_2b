...
parameters::parameters(int nt,int kt,int nk,int ntau,int size,double beta,double h,double mu,double den, double epsilon, double xi,std::vector<double> &delta,double v01,int suscep,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &omega0,CFUNC &g,std::vector<double> &E,std::vector<double> &dE,double dipolRe,double dipolIm,double ratio,double fieldP,double fieldD,double mazza,bool update,double eta,double gamma,double mix,bool test,int omp_for_vie2,int phonontype,double bath_low,double bath_high,std::vector<double> &gC_bath,std::vector<double> &gV_bath,int migdal,std::vector<double> &gBATH,std::vector<double> &omegaBATH): 
		nt(nt), ntau(ntau), size(size), nk(nk), kt(kt), beta(beta), h(h), mu(mu), den(den), epsilon(epsilon),xi(xi),delta(delta),v01(v01),suscep(suscep),
...
int main(int argc,char *argv[]){
	...
	std::vector<double> U,V,tt,delta,Ak,dAk,gVec,gBATHVec,omega0Vec;
	approx<lattice_1d_2b_nofield_deb> *lattice;
	int approximation;
	double den;
	// To fix after  
	int suscep=0;
	std::vector<double> E,dE;
	std::vector<double> gC_bath(nt,0.0),gV_bath(nt,0.0),omegaBATH(nt,1.0);
	double dipolRe=0.0,dipolIm=0.0,fieldP=0.0,fieldD=0.0,mazza=0.0;
	bool update="true";
	double eta=0.0;
	int phonontype=1;
	double bath_low=0.5,bath_high=0.7;
	int migdal=1;
	double ratio=1.0,gamma=0.0;
...
&ntasks);
	...
	// init the lattice and the physical input parameters
	{
		...
		find_param_tvector(argv[1],"__gBATH=",gBATHVec,nt);
		...
		find_param_tvector(argv[1],"__Ak=",E,nt);
		find_param_tvector(argv[1],"__dAk=",dE,nt);
	...
	CFUNC g(nt,1),gBATH(nt,1),omega0(nt,1);
	{
	  for(int tstp=-1;tstp<=nt;tstp++){
	    cdmatrix tmpG(1,1),tmpOm(1,1),tmpGBATH(1,1);
	    tmpG(0,0)=gVec[tstp+1];
	    tmpOm(0,0)=omega0Vec[tstp+1];
	    tmpGBATH(0,0)=gBATHVec[tstp+1];
	    g.set_value(tstp,tmpG);
	    gBATH.set_value(tstp,tmpGBATH);
	    omega0.set_value(tstp,tmpOm);
	  }
	}
	
	parameters param(nt,kt,nk,ntau,size,beta,h,mu,den,epsilon,xi,delta,v01,suscep,tt,U,V,omega0,g,E,dE,dipolRe,dipolIm,ratio,fieldP,fieldD,mazza,update,eta,gamma,mix,test,(omp_for_vie2==1 ? true : false),phonontype,bath_low,bath_high,gC_bath,gV_bath,migdal,gBATHVec,omegaBATH);
	switch(approximation){
		// case 0 : lattice=new mpi_lattice_step<lattice_1d_2b_nofield_deb>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,gBATH,xi,delta,Ak,dAk,v01,nk,kt);break;
		// case 1 : lattice=new mpi_lattice_step_GKBA<lattice_1d_2b_nofield,phonon_dipol>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,xi,delta,Ak,dAk,v01,nk,kt);break;
		// case 2 : lattice=new mpi_lattice_step_RPA<lattice_1d_2b_nofield>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,gBATH,xi,delta,Ak,dAk,v01,nk,kt,test,mix);break;
		case 3 : lattice=new mpi_lattice_step_2b_deb<lattice_1d_2b_nofield_deb>(param);break;
	}
	// TODO
	if(restart_time!=-1){
	...
	for(tstp=restart_time;tstp<=nt;tstp++){
		...
		for(iter=1;iter<=itermax1;iter++){
		  err=lattice->step(tstp,iter,kt,0.0,0.0,0.0); /// should work at time zero!
		  if(tid==0){
		  	cdmatrix tmp;
	...
	std::cout << "Print " << print_k << " " << tid << std::endl;
	char filename[1000];
