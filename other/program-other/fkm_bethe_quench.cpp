...
int main(int argc,char *argv[]){
    ...
    int nt=std::stoi(argv[4]),ntau=std::stoi(argv[5]),tstp,itermax=100,iter_rtime=5,kt=5,size,sig;
    double x1=0.0,x2=1.0;
	double U0=atof(argv[1]),U1=atof(argv[2]),beta=5.0,h=atof(argv[3]),mu,err,errmax=1e-9;
	...
	for(tstp=0;tstp<=nt;tstp++) f0[tstp]=-0.5*((U0+U1)/2.0+(U1-U0)/2.0*erf(5.922*(tstp*h-(x1+x2)/2.0)/((x2-x1)/2.0)));
	f1[-1]=U0*0.5;
	//for(tstp=0;tstp<=nt;tstp++) f1[tstp]=U1*0.5;		
	for(tstp=0;tstp<=nt;tstp++) f1[tstp]=0.5*((U0+U1)/2.0+(U1-U0)/2.0*erf(5.922*(tstp*h-(x1+x2)/2.0)/((x2-x1)/2.0)));
	ofstream myfile;
	myfile.open("quench.txt");
	for(tstp=-1;tstp<=nt;tstp++){
		myfile << tstp*h << " " << std::real(f0[tstp+1]) << " " << std::real(f1[tstp+1]) << std::endl;
	}
	myfile.close();
	...
	for(int iter=0;iter<=itermax;iter++){
		cntr::dyson_mat(G0,Delta,mu,f0,integration::I<double>(kt), beta, CNTR_MAT_FIXPOINT);	
		cntr::dyson_mat(G1,Delta,mu,f1,integration::I<double>(kt), beta, CNTR_MAT_FIXPOINT);	
	...
	for(tstp=kt+1;tstp<=nt;tstp++){
		cntr::herm_matrix_timestep<double> gtemp1(tstp,ntau,size,-1);
		...
		for(int iter=1;iter<=iter_rtime;iter++){
			...
			G.smul(tstp,0.5);
			G.incr_timestep(tstp,G1,0.5);
			cout << "TSTP iteration : " << iter << " |G-Gtemp|= " << cntr::distance_norm2(tstp,G,gtemp1) << endl;
			gtemp1.set_timestep(tstp,G);
			Delta.set_timestep(tstp,G); 
		}
	}
    ...
    group_id = create_group(file_id, "G0");
    store_herm_greens_function(group_id, G0);
    group_id = create_group(file_id, "G1");
    store_herm_greens_function(group_id, G1);
