...
lattice_1d_2b_nofield::lattice_1d_2b_nofield(void){
}

lattice_1d_2b_nofield::lattice_1d_2b_nofield(int nk,int nt,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &g,CFUNC &omega0,CFUNC &gbath,double xi,std::vector<double> &delta,std::vector<double> &A,std::vector<double> &dA,double v01,double mu,int size,int kt,double h):
...
int lattice_1d_2b_nofield::representative_kk(int kbz){
  assert(kbz<=nk_bz_-1);	
  return ( kbz <= G_ ? kbz  : 2*G_-kbz);
}

int lattice_1d_2b_nofield::add_kpoints(int k1,int s1,int k2,int s2){
...
void lattice_1d_2b_nofield::init_kk(int nk){
	double dk,kw;
	assert(nk>1);
	nk_=nk;
	G_=nk_-1;
...
double lattice_1d_2b_nofield::V(int tstp,double qq,int j1,int j2){
	if(j1!=j2){
		double xi2=xi_*xi_;
		double cq=cos(qq);
...
std::complex<double> lattice_1d_2b_nofield::Velph(int tstp,std::complex<double> &X,int j1,int j2){
	if(j1!=j2){
		return g_[tstp+1]*X;
	}else{
		return 0.0;
...
void lattice_1d_2b_nofield::V(cdmatrix &Vmatrix,int tstp,double qq){
...
void lattice_1d_2b_nofield::Velph(cdmatrix &Vmatrix,int tstp,std::complex<double> &X){
		// not efficient, but used only for initialization
	Vmatrix.resize(nrpa_,nrpa_);
	for(int j1=0;j1<nrpa_;j1++){
		for(int j2=0;j2<nrpa_;j2++){
...
void lattice_1d_2b_nofield::hk(cdmatrix &hkmatrix,int tstp,double kk,int iter){
...
void lattice_1d_2b_nofield::hkfree(cdmatrix &hkmatrix,int tstp,double kk){
	double epsk=-2.0*tt_[tstp+1]*cos(kk);
	double epsk1=2.0*tt_[tstp+1]*cos(kk);
	double vq0_00=V(tstp,0.0,0,0);
	double vq0_01=V(tstp,0.0,0,1);
...
void lattice_1d_2b_nofield::vk(cdmatrix &vkmatrix,int tstp,double kk){
...
// Dipolar interaction
void lattice_1d_2b_nofield::Ak(cdmatrix &hkmatrix,int tstp,double kk){
	cdouble ak=A_[tstp+1];
	hkmatrix.resize(nrpa_,nrpa_);
	hkmatrix.setZero();
...
// dAk interaction
void lattice_1d_2b_nofield::dAk(cdmatrix &hkmatrix,int tstp,double kk){
