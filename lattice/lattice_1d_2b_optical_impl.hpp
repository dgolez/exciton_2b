...
lattice_1d_2b_optical_nofield::lattice_1d_2b_optical_nofield(void){
}

lattice_1d_2b_optical_nofield::lattice_1d_2b_optical_nofield(parameters &param):
...
void lattice_1d_2b_optical_nofield::efield_to_afield(int nt,double h,std::vector<double> &efield,std::vector<double> &afield,int kt){
...
int lattice_1d_2b_optical_nofield::add_kpoints(int k1,int s1,int k2,int s2){
...
void lattice_1d_2b_optical_nofield::init_kk(int nk){
	double dk,kw;
	assert(nk>1);
	nk_=nk;
	G_=nk_/2;
...
double lattice_1d_2b_optical_nofield::V(int tstp,double qq,int j1,int j2){
	if(j1!=j2){
		double xi2=xi_*xi_;
		double cq=cos(qq);
...
std::complex<double> lattice_1d_2b_optical_nofield::Velph(int tstp,std::complex<double> &X,int j1,int j2){
	if(j1!=j2){
		return g_[tstp+1]*X;
	}else{
		return 0.0;
...
void lattice_1d_2b_optical_nofield::V(cdmatrix &Vmatrix,int tstp,double qq){
...
void lattice_1d_2b_optical_nofield::Velph(cdmatrix &Vmatrix,int tstp,std::complex<double> &X){
		// not efficient, but used only for initialization
	Vmatrix.resize(nrpa_,nrpa_);
	for(int j1=0;j1<nrpa_;j1++){
		for(int j2=0;j2<nrpa_;j2++){
...
void lattice_1d_2b_optical_nofield::hk(cdmatrix &hkmatrix,int tstp,double kk,int iter){
	// Diagonal terms
	double kkshift=kk+A_[tstp+1]*fieldP_;
	double epsk=-2.0*tt_[tstp+1]*cos(kkshift);
	...
	double dbar1=-0.5*(vq0_00+vq0_01)+2.0*tt_[tstp+1]*cos(0.0)*ratio_;
	double d0=dbar0-0.5*delta_[tstp+1];
	double d1=dbar1+0.5*delta_[tstp+1];
	// std::cout << " Inter " << vq0_00 << " " << vq0_01 << " " << dbar0 << " " << dbar1 <<" " << d0 << " " << d1 << std::endl;
	//double maz=mazza_*(1.0-cos(kkshift));
	std::complex<double> I(0.0,1.0);
	std::complex<double> maz=2.0*I*mazza_*exp(I*kk/2.0)*sin(kk/2.0);

	hkmatrix.resize(nrpa_,nrpa_);
	hkmatrix.setZero();
	hkmatrix(0,0)=-epsk+d0;
	hkmatrix(1,1)=epsk*ratio_+d1;
	hkmatrix(0,1)=ak+maz;
	...
	// std::cout << "Herm1 " << kk << " " << " " << hkmatrix << std::endl;
	// std::cout << "Herm2 " << kk << " " << " " << epsk << " " << vq0_00  << " " << vq0_01 << std::endl;
	// std::cout << "Herm2 " << hkmatrix <<  " " << " " << vq0_00 << " " << vq0_01 << " " << dbar0 << " " << dbar1  << std::endl;
}

void lattice_1d_2b_optical_nofield::hkfree(cdmatrix &hkmatrix,int tstp,double kk){
	// Diagonal terms
	double kkshift=kk;
...
void lattice_1d_2b_optical_nofield::vk(cdmatrix &vkmatrix,int tstp,double kk){
	// Diagonal terms
...
//Velocity is still 1d vector
void lattice_1d_2b_optical_nofield::vkFULL(cdmatrix &vkmatrix,int tstp,double kk){
	// Diagonal terms
	double kkshift=kk+A_[tstp+1];
	double vk=2.0*tt_[tstp+1]*sin(kkshift);
...
// Dipolar interaction
void lattice_1d_2b_optical_nofield::Ak(cdmatrix &hkmatrix,int tstp,double kk){
        double kkshift=kk;
	cdouble ak=dipol_;
...
// dAk interaction
void lattice_1d_2b_optical_nofield::dAk(cdmatrix &hkmatrix,int tstp,double kk){
