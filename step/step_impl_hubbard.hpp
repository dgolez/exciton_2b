...
mpi_lattice_step_hubbard<LATTICE>::mpi_lattice_step_hubbard(parameters_hub &param){
...
template <class LATTICE>
mpi_lattice_step_hubbard<LATTICE>::mpi_lattice_step_hubbard(parameters_tns &param){
	epsilon_=param.epsilon;
	nt_=param.nt;
	ntau_=param.ntau;
	beta_=param.beta;
	...
	// /////////////////////////////////////////////////
	for(int k=0;k<nk_;k++){
		density_k_[k]=kpoint_density<LATTICE>(nt_,ntau_,param.size,beta_,h_,latt_.kpoints_[k],latt_,param.mu,param.den,param.epsilon,param.gamma,param.mix,0);
	  // set the vertex at *every* kk
	  vertex_[k]=CFUNC(nt_,nrpa_);
	  for(int tstp=-1;tstp<=nt_;tstp++){
			cdmatrix vtmp;
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::step(int tstp,int iter,int kt,double om0,double s,double amp){
  assert(tstp<=nt_);
  int n;
...
template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::extrapolate_rho(int tstp){
	if(tstp<-1) return;
	if(tstp==0){
		for(int k=0;k<nk_;k++){
			cdmatrix tmp;
...
template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::step_boson(int tstp){
	// Set phonons
...
template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::get_Sigma_Hartree(int tstp,int kk,CFUNC &S){
	cdmatrix stmp(nrpa_,nrpa_),tmp(nrpa_,nrpa_),v0;
	if(update_ || tstp<0){
	// kk-independent, nut anyway
	stmp.setZero();
	//// the vertex V_{a1,a2} = vertex(q=0)_{a1,a2}
	rho_loc_.get_value(tstp,tmp);
	// std::cout << " -------------------------------- " << std::endl;
	// std::cout << "Hartree Density " << tstp << " " << tmp <<  std::endl;
	...
	for(int k=0;k<latt_.nk_;k++){
		...
		for(int i1=0;i1<nrpa_;i1++){
			for(int i2=0;i2<nrpa_;i2++){
				if(i1==i2){
					// std::cout << "Hartree  intra " << i1 << " " << i2 << " " << k << " " << v0(i1,i2) << " " << rtmp(i2,i2)  <<  std::endl;
					stmp(i1,i1)+=v0(i1,i2)*rtmp(i2,i2)*wk;
					// std::cout << "Hartree  intra " << stmp(i1,i1) << std::endl;
				}else if(i2!=i1){
					// std::cout << "Hartree  inter " << i1 << " " << i2 << " " << k << " " << v0(i1,i2) << " " << rtmp(i2,i2) <<  std::endl;
					stmp(i1,i1)+=v0(i1,i2)*rtmp(i2,i2)*wk*2.0;
					// std::cout << "Hartree  inter " << stmp(i1,i1) << std::endl;
				}
			}
		}
	}
	...
	// std::cout << "Hartree phonon " << tstp << " " << Vtmp << " " << Xtmp<< std::endl;
	stmp+=Vtmp;
	...
	S.set_value(tstp,stmp);
	// std::cout << "Hartree " << tstp << " " << stmp <<  std::endl;
	// std::cout << " -------------------------------- " << std::endl;
}

template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::set_local(int tstp){
...
template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::get_Sigma_Fock(int tstp,int kk,CFUNC &S){
	...
	for(int q=0;q<latt_.nk_;q++){
		...
		for(int i1=0;i1<nrpa_;i1++){
			for(int i2=0;i2<nrpa_;i2++){
				// This is just a piecewise product - implement it as in dp
				if(i1==i2){
					stmp(i1,i2)-=0.0; // Zero intraband-local because of the spin conservation + Paulli exclusion principle
				}else{
					stmp(i1,i2)-= vtmp(i1,i2)*rtmp(i1,i2)*wk;
				}
			}
		}
		// std::cout << "stmpr q " <<  stmp << " " << wk << std::endl;
		// std::cout <<" ------------- " << std::endl;
	}
	inter_tmp=stmp;
	}else{
	...
	S.set_value(tstp,stmp);
	// std::cout << " -------------------------------- " << std::endl;
	// std::cout << "Fockk " << tstp << " " << stmp <<  std::endl;
}

template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_ekin(int tstp){
	cdmatrix rtmp,hktmp;
	...
	if(tid_==tid_root_){
		...
		// MPI::COMM_WORLD.Bcast(&ekin,1,MPI::DOUBLE,tid_root_);
	#endif
	return ekin;
}
...
template <class LATTICE>
void mpi_lattice_step_hubbard<LATTICE>::get_ekin(int tstp,cdmatrix &ekin){
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_curr(int tstp){
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_curr_peierls(int tstp){
	cdmatrix rtmp,hktmp,Aktmp,rtmp1,rtmp2,drtmp;
	double vktmp1=0.0,vktmp2=0.0,vk=0.0;
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_curr_dip(int tstp){
	cdmatrix rtmp,hktmp,Aktmp,rtmp1,rtmp2,drtmp;
	double vktmp1=0.0,vktmp2=0.0,vk=0.0;
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_dip(int tstp){
...
// template <class LATTICE>
// std::complex<double> mpi_lattice_step_hubbard<LATTICE>::get_chi0(CFUNC &chi,double domega,int nomega,double om0,double s,double amp){
...
template <class LATTICE>
std::complex<double> mpi_lattice_step_hubbard<LATTICE>::get_chi0(double omega,int mu,int nu,double om0,double s,double amp){
  cdmatrix rtmp(2,2),tmp(2,2);
  std::complex<double> optical0=std::complex<double>(0.0,0.0);
  std::vector<cdmatrix> sigma(4);
  double eta=0.01;
  ...
  // add all to task tid
  //#if FLEX_USE_MPI==1
  //	MPI::COMM_WORLD.Bcast(&vk,1,MPI::DOUBLE,tid_root_);
  //#endif
  //return vk;
}
...
template <class LATTICE>
std::complex<double> mpi_lattice_step_hubbard<LATTICE>::get_optical0(CFUNC &optics,double domega,int nomega,double om0,double s,double amp){
  cdmatrix rtmp(2,2),tmp(2,2);
  std::complex<double> optical0=std::complex<double>(0.0,0.0);
  double om;
  std::vector<cdmatrix> sigma(4);
...
template <class LATTICE>
std::complex<double> mpi_lattice_step_hubbard<LATTICE>::get_seebeck(CFUNC &seebeck,CFUNC &omega,double domega,int nomega){
...
template <class LATTICE>
std::complex<double> mpi_lattice_step_hubbard<LATTICE>::get_dos(CFUNC &dos,double domega,int nomega){
  std::complex<double> phi=0.0;
  cdmatrix previous(1,1);
  double omega;
  dvector ek(2);
...
#define EXPMAX 100
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::dfermi(double omega,double beta){
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_seebeck_boltzmann(double beta){
	double nom=0,mu;
	double denom=0;
	int nk=latt_.nk_;
	double dk=2.0*PI/nk;
...
template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_dAk(int tstp){
	...
	// add all to task tid
		...
		// MPI::COMM_WORLD.Bcast(&vktmp,1,MPI::DOUBLE,tid_root_);
	#endif
	return vk;
}

template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::gauss(double om,double om0,double s){
	return exp(-(om-om0)*(om-om0)/(2*s*s))/(sqrt(2*PI)*s);
}

template <class LATTICE>
double mpi_lattice_step_hubbard<LATTICE>::get_eneHF(int tstp){
	...
	//    MPI::COMM_WORLD.Allreduce(&eHFtmp,&eHF,1,MPI::DOUBLE,MPI_SUM);
	#endif
	return eHF;
}
