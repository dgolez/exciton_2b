...
mpi_lattice_step_cdw<LATTICE>::mpi_lattice_step_cdw(int nt,int ntau,int size,double beta,double h,bool use_omp_for_vie2,double mu,double epsilon,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &g,CFUNC &omega0,double xi,std::vector<double> &delta,std::vector<double> &A,std::vector<double> &dA,double v01,int nk,int kt){
	epsilon_=epsilon;
	nt_=nt;
	ntau_=ntau;
	beta_=beta;
	...
	latt_=LATTICE(nk,nt,tt,U,V,g,omega0,xi,delta,A,dA,v01,mu,size,kt,h);
	...
	/////////////////////////////////////////////////
	for(int k=0;k<nk_;k++){
		density_k_[k]=kpoint_density<LATTICE>(nt,ntau,size,beta,h,latt_.kpoints_[k],latt_,mu,epsilon);
...
template <class LATTICE>
double mpi_lattice_step_cdw<LATTICE>::step(int tstp,int iter,int kt,double om0,double s,double amp){
  assert(tstp<=nt_);
  int n;
...
template <class LATTICE>
void mpi_lattice_step_cdw<LATTICE>::extrapolate_rho(int tstp){
	if(tstp<-1) return;
	if(tstp==0){
		for(int k=0;k<nk_;k++){
			cdmatrix tmp;
...
template <class LATTICE>
void mpi_lattice_step_cdw<LATTICE>::step_boson(int tstp){
	// Set phonons
...
template <class LATTICE>
void mpi_lattice_step_cdw<LATTICE>::get_Sigma_Hartree(int tstp,int kk,CFUNC &S){
	...
	for(int k=0;k<latt_.nk_rbz_;k++){
		...
		for(i1=0;i1<nrpa_;i1++){
			for(i2=0;i2<nrpa_;i2++){
				...
				cdouble stmpij=0.0;
				// std::cout << " Hartreetmp : " << a1 << " " << a2 << " " << wk << " " << latt_.nk_rbz_ << " " << nrpa_ << " " << norb_   << std::endl;
				...
				if(a1==a2){
					for(at=0;at<norb_;at++){
						for(ct=0;ct<2;ct++){
							// std::cout << "Index " << a1 <<  " " << c12 << " " << at << " " << c2 << " " << ct << " " << latt_.idx(a1,c12) << " " << latt_.idx(at,c12) << " " << latt_.idx(at,(c2+ct)%2) << " " << latt_.idx(at,(c1+ct)%2) << std::endl;
							stmpij+=v0(latt_.idx(a1,c12),latt_.idx(at,c12))*
								        rtmp(latt_.idx(at,(c2+ct)%2),latt_.idx(at,(c1+ct)%2));
						}
					}
				}
				// std::cout << "stmp " << i1 << " " << i2 << " " << stmpij << std::endl;
...
template <class LATTICE>
void mpi_lattice_step_cdw<LATTICE>::get_Sigma_Fock(int tstp,int kk,CFUNC &S){
...
template <class LATTICE>
void mpi_lattice_step_cdw<LATTICE>::set_local(int tstp){
	...
	for(int k=0;k<latt_.nk_rbz_;k++){
		...
		loc+=rtmp*wk;
		// std::cout << "Set loc in " << rtmp << " " << wk << " "<< loc << std::endl;
	}
	rho_loc_.set_value(tstp,loc);
	// std::cout << "Set local " << loc << std::endl;
}


template <class LATTICE>
double mpi_lattice_step_cdw<LATTICE>::get_ekin(int tstp){
	cdmatrix rtmp,hktmp;
...
template <class LATTICE>
double mpi_lattice_step_cdw<LATTICE>::get_curr(int tstp){
	cdmatrix rtmp,hktmp;
...
template <class LATTICE>
double mpi_lattice_step_cdw<LATTICE>::get_dAk(int tstp){
	cdmatrix rtmp,hktmp;
	double vktmp=0.0,vk=0.0;
	if(tid_==tid_root_){
		for(int k=0;k<latt_.nk_rbz_;k++){
...
template <class LATTICE>
double mpi_lattice_step_cdw<LATTICE>::get_eneHF(int tstp){
