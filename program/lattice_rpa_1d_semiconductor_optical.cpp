...
int main(int argc,char *argv[]){
	...
	bool test,update;
	double mu,beta,h,errmax,errmax_rtime,err,v01,mazza,epsilon,mix,gamma,den;
	...
	approx<lattice_1d_2b_optical_nofield> *lattice;
	...
	// std::cout << "Tu smo1 "<< std::endl;
	parameters param(nt,kt,nk,ntau,size,beta,h,mu,den,epsilon,xi,delta,v01,suscep,tt,U,V,omega0,g,E,dE,dipolRe,dipolIm,ratio,fieldP,fieldD,mazza,update,eta,gamma,mix,test,(omp_for_vie2==1 ? true : false),phonontype,bath_low,bath_high,gC_bath,gV_bath,migdal,gBATH,omegaBATH);
	// std::cout << "Tu smo2 " << std::endl;
	switch(approximation){
	  // TOdo change also for gw and 2b
	  case 0 : lattice=new mpi_lattice_step_optical<lattice_1d_2b_optical_nofield>(param);break;
	  case 3 : lattice=new mpi_lattice_step_2b_optical<lattice_1d_2b_optical_nofield>(param);break;
	...
	for(tstp=restart_time;tstp<=nt;tstp++){
		...
		// MPI_Bcast(&cont, 1, MPI_INTEGER,cont, MPI_COMM_WORLD);
		if(!cont){
		  if(tid==0){
