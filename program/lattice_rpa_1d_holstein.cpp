...
// TODO el-ph g and omega0 is assumed constant in time, see step_impl_phonon.hpp
...
int main(int argc,char *argv[]){
	int tid,ntasks,ntau,nt,itermax,iter_rtime,omp_for_vie2,kt,nk,tstp=-2,iter,print_k,outputfrequency,size=1;
	int restart_time,cont,use_rpa=true;
	double mu,beta,h,errmax,errmax_rtime,err,v01,epsilon;
	double wtime_limit,wtime_start,wtime_end,xi;
	std::vector<double> U,V,tt,delta,Ak,dAk,g,omega0;
	// std::unique_ptr<approx<lattice_1d_nofield> > lattice;
	approx<lattice_1d_1b_nofield,phonon_hol> *lattice;
	bool test;
	int approximation; 
...
&ntasks);
	...
	// init the lattice and the physical input parameters
	{
		...
		find_param_tvector(argv[1],"__omega0=",omega0,nt);
	...
	switch(approximation){
		case 0 : lattice=new mpi_lattice_step<lattice_1d_1b_nofield,phonon_hol>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,xi,delta,Ak,dAk,v01,nk,kt);break;
		case 1 : lattice=new mpi_lattice_step_GKBA<lattice_1d_1b_nofield,phonon_hol>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,xi,delta,Ak,dAk,v01,nk,kt);break;
	case 2 : lattice=new mpi_lattice_step_RPA<lattice_1d_1b_nofield,phonon_hol>(nt,ntau,size,beta,h,(omp_for_vie2==1 ? true : false),mu,epsilon,tt,U,V,g,omega0,xi,delta,Ak,dAk,v01,nk,kt,test);break;
	}
	// TODO
	if(restart_time!=-1){
	...
	for(tstp=restart_time;tstp<=nt;tstp++){
		...
		for(iter=1;iter<=itermax1;iter++){
		  err=lattice->step(tstp,iter,kt1,0.0,0.0); /// should work at time zero!
		  if(tid==0){
		  	cdmatrix tmp;
		  	lattice->rho_loc_.get_value(tstp,tmp);
		  	cout << "tstp= " << tstp << " iter:  " << iter << " err: " << err << " - " << tmp << endl;
		  }
		  if(err<errmax1 ){
		  	break;
		  }
