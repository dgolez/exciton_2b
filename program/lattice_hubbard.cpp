...
parameters_hub::parameters_hub(int nt,int kt,int nk,int ntau,int size,double beta,double h,double mu,double den, double epsilon, double xi,std::vector<double> delta,double v01,int suscep,std::vector<double> &tt,std::vector<double> &U,std::vector<double> &V,CFUNC &omega0,CFUNC &g,std::vector<double> &E,std::vector<double> &dE,double dipolRe,double dipolIm,double ratio,double fieldP,double fieldD,double mazza,bool update,double eta,double gamma,double mix,bool test,int omp_for_vie2,int phonontype,double bath_low,double bath_high,std::vector<double> gC_bath,std::vector<double> gV_bath,int migdal): 
		...
		tt(tt),U(U),V(V),omega0(omega0),g(g),E(E),dE(dE),dipol(dipolRe,dipolIm),ratio(ratio),fieldP(fieldP),fieldD(fieldD),mazza(mazza),update(update),eta(eta),gamma(gamma),mix(mix),test(test),omp_for_vie2(omp_for_vie2),phonontype(phonontype),bath_low(bath_low),bath_high(bath_high),gC_bath(gC_bath),gV_bath(gV_bath),migdal(migdal){};
...
int main(int argc,char *argv[]){
	...
	std::vector<double> U,V,tt,delta,E,dE,gVec,omega0Vec;
...
&ntasks);
	...
	parameters_hub param(nt,kt,nk,ntau,size,beta,h,mu,den,epsilon,xi,delta,v01,suscep,tt,U,V,omega0,g,E,dE,dipolRe,dipolIm,ratio,fieldP,fieldD,mazza,update,eta,gamma,mix,test,(omp_for_vie2==1 ? true : false),phonontype,bath_low,bath_high,gC_bath,gV_bath,migdal);
	switch(approximation){
	  // TOdo change also for gw and 2b
	  case 0 : lattice=new mpi_lattice_step_hubbard<lattice_1d_hubbard>(param);break;
	  case 3 : lattice=new mpi_lattice_step_hubbard_RPA<lattice_1d_hubbard>(param);break;
