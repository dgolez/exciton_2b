...
int main(int argc,char *argv[]){
	...
	double U0=atof(argv[1]),U1=atof(argv[2]),beta=5.0,h=atof(argv[3]),mu,err,errmax=1e-9,alpha=atof(argv[6]);
	...
	for(tstp=0;tstp<=nt;tstp++) f0[tstp]=-0.5*(U0+U1*sin((U0+alpha*tstp*h)*tstp*h));
	f1[-1]=U0*0.5;
	//for(tstp=0;tstp<=nt;tstp++) f1[tstp]=U1*0.5;		
	for(tstp=0;tstp<=nt;tstp++) f1[tstp]=0.5*(U0+U1*sin((U0+alpha*tstp*h)*tstp*h));
	ofstream myfile;
